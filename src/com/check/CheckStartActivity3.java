...
public class CheckStartActivity3 extends ActionBarActivity {
	...
	private String succeed=null;
		...
		protected void onCreate(Bundle savedInstanceState) {
			...
			setContentView(R.layout.activity_check_start3);
		    ipData helper=new ipData(CheckStartActivity3.this);
			ipUser ipuser=helper.query(1);
			ip=ipuser.getNumber();	
			connectURL="http://"+ip+":8088/elevatorData/checkSpeed3.php";
			Intent i=getIntent();
		...
		{
			TextView carSafeView=(TextView)findViewById(R.id.overSpeed);
			carSafeView.setText(R.string.OverSpeed);
	        	...
	        	switch(msg.what)
				{
					...
					default:
						...
						if("succeed".equals(succeed))
			        	{
							...
							Toast.makeText(CheckStartActivity3.this, "提交成功", Toast.LENGTH_LONG).show();
						
							Intent intent=new Intent();
							intent.setClass(CheckStartActivity3.this, deal.class);
							intent.putExtra("ok", "check");
							CheckStartActivity3.this.startActivity(intent);
						
						}else{
							Toast.makeText(CheckStartActivity3.this, "提交失败", Toast.LENGTH_LONG).show();
						}
	    ...
	    class nextListener implements android.view.View.OnClickListener {
	    	...
	    	public void onClick(View v) {
	    		...
	    		if (TextUtils.isEmpty(reportNo) ) {  
	                Toast.makeText(CheckStartActivity3.this, "请填写完整", Toast.LENGTH_LONG).show();
	    		}else{
	    			new Thread() {
	    	            @Override
	    	            public void run() { 
